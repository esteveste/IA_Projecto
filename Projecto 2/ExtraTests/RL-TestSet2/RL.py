...
class finiteMDP:
    ...
    def __init__(self, nS, nA, gamma, P=[], R=[], absorv=[],lr=0.7):
        self.nS = nS
        self.nA = nA
        self.gamma = gamma
    ...
    def traces2Q(self, trace):
        ...
        for step in trace:
            s,a,s_prime,r = step.astype(int)
            self.Q[s,a] = self.Q[s,a] + self.lr * (r + self.gamma*np.max(self.Q[s_prime]) - self.Q[s,a])

        return self.Q
    
    def policy(self, s, poltype = 'exploration', par = []):
        # implementar esta funcao
